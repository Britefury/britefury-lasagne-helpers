...
def _load_cifar10(filename='cifar-10-python.tar.gz'):
    ...
    data_paths = [os.path.join(dataset.DATA_DIR, 'cifar-10-batches-py', 'data_batch_{}'.format(i)) for i in range(1, 6)]
    test_path = os.path.join(dataset.DATA_DIR, 'cifar-10-batches-py', 'test_batch')
    ...
    # Load them
    data_batches = [cPickle.load(open(p, 'rb')) for p in data_paths]
    test_batch = cPickle.load(open(test_path, 'rb'))
