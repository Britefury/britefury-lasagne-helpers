...
def _load_stl(filename='stl10_binary.tar.gz', load_unlabeled=False):
    ...
    train_X_path = os.path.join(dataset.DATA_DIR, 'stl10_binary', 'train_X.bin')
    train_y_path = os.path.join(dataset.DATA_DIR, 'stl10_binary', 'train_y.bin')
    test_X_path = os.path.join(dataset.DATA_DIR, 'stl10_binary', 'train_X.bin')
    test_y_path = os.path.join(dataset.DATA_DIR, 'stl10_binary', 'train_y.bin')
    unlabeled_X_path = os.path.join(dataset.DATA_DIR, 'stl10_binary', 'unlabeled_X.bin')
    class_names_path = os.path.join(dataset.DATA_DIR, 'stl10_binary', 'class_names.txt')
    fold_indices_path = os.path.join(dataset.DATA_DIR, 'stl10_binary', 'fold_indices.txt')
    ...
    if not unpacked:
        print('unpacking')
        tarfile.open(name=path, mode='r:gz').extractall(path=dataset.DATA_DIR)
