...
def _build_simple_block(incoming_layer, names,
...
def _build_residual_block(incoming_layer, ratio_n_filter=1.0, ratio_size=1.0, has_left_branch=False,
                          upscale_factor=4, ix=''):
    ...
    # right branch
    right_tail = _build_simple_block(
...
class ResNet50Model (imagenet.AbstractImageNetModel):
    ...
    def build_network(cls, input_shape=None):
        ...
        for c in block_size:
            if c == 'a':
                net = _build_residual_block(net, 1, 1, True, 4, ix='2{}'.format(c))
            else:
                net = _build_residual_block(net, 1.0 / 4, 1, False, 4, ix='2{}'.format(c))

        block_size = list('abcd')
        for c in block_size:
            if c == 'a':
                net = _build_residual_block(net, 1.0 / 2, 1.0 / 2, True, 4, ix='3{}'.format(c))
            else:
                net = _build_residual_block(net, 1.0 / 4, 1, False, 4, ix='3{}'.format(c))

        block_size = list('abcdef')
        for c in block_size:
            if c == 'a':
                net = _build_residual_block(
                    net, 1.0/2, 1.0/2, True, 4, ix='4{}'.format(c))
            else:
                net = _build_residual_block(net, 1.0 / 4, 1, False, 4, ix='4{}'.format(c))

        block_size = list('abc')
        for c in block_size:
            if c == 'a':
                net = _build_residual_block(net, 1.0 / 2, 1.0 / 2, True, 4, ix='5{}'.format(c))
            else:
                net = _build_residual_block(net, 1.0 / 4, 1, False, 4, ix='5{}'.format(c))
